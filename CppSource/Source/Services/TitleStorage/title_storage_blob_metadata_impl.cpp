...
void XSAPI_TITLE_STORAGE_BLOB_METADATA_IMPL::update(const xbox::services::title_storage::title_storage_blob_metadata& cppObj)
{
    ...
    m_blobPath = utils::to_utf8string(cppObj.blob_path());
    ...
    m_displayName = utils::to_utf8string(cppObj.display_name());
    ...
    m_serviceConfigurationId = utils::to_utf8string(cppObj.service_configuration_id());
    m_pMetadata->serviceConfigurationId = m_serviceConfigurationId.data();

    m_xboxUserId = utils::to_utf8string(cppObj.xbox_user_id());
