...
{
    m_name = utils::to_utf8string(m_cppAssociation.name());
...
{
    m_id = utils::to_utf8string(m_cppRequirement.id());
    m_cRequirement->id = m_id.c_str();

    m_currentProgressValue = utils::to_utf8string(m_cppRequirement.current_progress_value());
    m_cRequirement->currentProgressValue = m_currentProgressValue.c_str();

    m_targetProgressValue = utils::to_utf8string(m_cppRequirement.target_progress_value());
...
{
    m_name = utils::to_utf8string(m_cppMediaAsset.name());
    m_cMediaAsset->name = m_name.c_str();

    m_uri = m_cppMediaAsset.url().to_string();
    m_url = utils::to_utf8string(m_uri);
...
{
    m_name = utils::to_utf8string(m_cppReward.name());
    m_cReward->name = m_name.c_str();

    m_description = utils::to_utf8string(m_cppReward.description());
    m_cReward->description = m_description.c_str();

    m_value = utils::to_utf8string(m_cppReward.value());
    m_cReward->value = m_value.c_str();

    m_valueType = utils::to_utf8string(m_cppReward.value_type());
...
{
    m_id = utils::to_utf8string(m_cppAchievement.id());
    m_cAchievement->id = m_id.c_str();

    m_serviceConfigurationId = utils::to_utf8string(m_cppAchievement.service_configuration_id());
    m_cAchievement->serviceConfigurationId = m_serviceConfigurationId.c_str();

    m_name = utils::to_utf8string(m_cppAchievement.name());
    ...
    {
        auto platform = utils::to_utf8string(cppPlatform);
    ...
    m_unlockedDescription = utils::to_utf8string(m_cppAchievement.unlocked_description());
    m_cAchievement->unlockedDescription = m_unlockedDescription.c_str();

    m_lockedDescription = utils::to_utf8string(m_cppAchievement.locked_description());
    m_cAchievement->lockedDescription = m_lockedDescription.c_str();

    m_productId = utils::to_utf8string(m_cppAchievement.product_id());
    ...
    m_deepLink = utils::to_utf8string(m_cppAchievement.deep_link());
